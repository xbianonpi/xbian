...
# Asks a question and requires yes or no as answer
function yesnoq() {
  if [ -n "$1" ]; then
    while [ 1 ]; do
      read -p "$1" YESNO
...
# Asks a yes or no question for max 3 times. If the answers is no, the
# script exists
function yesno() {
  COUNTER=0
  while [ $COUNTER -lt 3 ]; do
    ...
    if [ "${YESNO,,}" == "no" ]; then
      ...
      if [ $COUNTER -lt 2 ]; then echo Please answer yes or no; fi
    else
      break
    fi
    let COUNTER+=1
  ...
  if [ $COUNTER -eq "3" ]; then
    echo "First get your keys right before using this script."
    echo
...
if [ "$?" -ne "0" ]; then
  ...
  # Only when on an interactive shell. Don't ask after reboot, when ran 
  # from /etc/profile
  if [[ $- == *i* ]]; then
    # Continue? Dangerous actions follow below!
    yesno
  fi
...
if [ "$KERNELSIZE" -ne "$NEWSIZE" ]; then
  ...
  if [ "$KERNELSIZE" -ne "$NEWSIZE" ]; then
    ...
    # If we're not on an SSH connection, we can start this script
    # after a reboot
    if [ -z "$SSH_CONNECTION" ]; then
      yesnoq "Do you want the resize to continue automatically after reboot [yes|no]? "
      if [ ${YESNO,,} == "yes" ]; then
        # Copy this script to profile.d, so it will be executed when
        # the system reboots
	cp $0 /etc/profile.d/__resize.sh > /dev/null
	chmod +x /etc/profile.d/__resize.sh

        yesnoq "Do you want to reboot now [yes|no]? "
        if [ ${YESNO,,} == "yes" ]; then
          reboot
        fi
      fi
